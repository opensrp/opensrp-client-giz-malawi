...
public class OpdRegisterQueryProvider extends OpdRegisterQueryProviderContract {
    ...
    public String mainSelectWhereIDsIn() {
        ...
        String sqlQuery = "Select ec_child.id as _id, ec_child.first_name, ec_child.last_name, ec_child.middle_name, ec_child.gender, ec_child.dob, ec_child.home_address, 'Child' AS register_type, ec_child.relational_id AS relationalid, ec_child.zeir_id AS register_id, ec_child.last_interacted_with, ec_mother.first_name AS mother_first_name, ec_mother.last_name AS mother_last_name, ec_mother.middle_name AS mother_middle_name, opd_details.current_visit_start_date >= '$latest_start_visit_date' AS pending_diagnose_and_treat,  'ec_child' as entity_table, opd_details.current_visit_end_date FROM ec_child \n" +
                ...
                "UNION ALL \n" +
                "Select ec_mother.id as _id , first_name , last_name , middle_name , 'Female' AS gender , dob , home_address , 'ANC' AS register_type , relationalid , register_id , last_interacted_with , NULL AS mother_first_name , NULL AS mother_last_name , NULL AS mother_middle_name, opd_details.current_visit_start_date >= '$latest_start_visit_date' AS pending_diagnose_and_treat, 'ec_mother' as entity_table, opd_details.current_visit_end_date FROM ec_mother\n" +
                "    LEFT JOIN opd_details ON ec_mother.base_entity_id = opd_details.base_entity_id\n" +
                "    WHERE  ec_mother.id IN (%s) \n" +
                "UNION ALL \n" +
                "Select ec_client.id as _id , first_name , last_name , '' AS middle_name , gender , dob , '' AS home_address , 'OPD' AS register_type , relationalid , opensrp_id AS register_id , last_interacted_with , NULL AS mother_first_name , NULL AS mother_last_name , NULL AS mother_middle_name, opd_details.current_visit_start_date >= '$latest_start_visit_date' AS pending_diagnose_and_treat, 'ec_client' as entity_table, opd_details.current_visit_end_date FROM ec_client \n" +
