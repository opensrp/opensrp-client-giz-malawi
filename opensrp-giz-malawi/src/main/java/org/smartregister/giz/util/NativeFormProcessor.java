...
public class NativeFormProcessor {
    ...
    private JSONObject getOrCreateDetailsNode() throws JSONException {
        JSONObject formData = jsonForm.has(GizConstants.DETAILS) ? jsonForm.getJSONObject(GizConstants.DETAILS) : null;
        if (formData == null) {
            formData = new JSONObject();
            formData.put(GizConstants.Properties.FORM_VERSION, jsonForm.optString("form_version"));
            formData.put(GizConstants.Properties.APP_VERSION_NAME, BuildConfig.VERSION_NAME);

            jsonForm.put(GizConstants.DETAILS, formData);
        }
    ...
    public NativeFormProcessor tagTaskDetails(Task task) throws JSONException {
        ...
        formData.put(GizConstants.Properties.TASK_IDENTIFIER, task.getIdentifier());
        formData.put(GizConstants.Properties.TASK_BUSINESS_STATUS, task.getBusinessStatus());
        formData.put(GizConstants.Properties.TASK_STATUS, task.getStatus());
        return this;
    }
    ...
    public NativeFormProcessor tagLocationData(Location location) throws JSONException {
        ...
        JSONObject formData = getOrCreateDetailsNode();
        formData.put(GizConstants.Properties.LOCATION_ID, location.getId());
        formData.put(GizConstants.Properties.LOCATION_UUID, location.getId());
        formData.put(GizConstants.Properties.LOCATION_VERSION, location.getServerVersion().toString());
        return this;
    }

    public NativeFormProcessor tagLocationId(String locationId) throws JSONException {
        jsonForm.put(GizConstants.DETAILS, locationId);
        return this;
    }

    public NativeFormProcessor tagChildLocationId(String childLocationId) throws JSONException {
        jsonForm.put(GizConstants.DETAILS, childLocationId);
        return this;
    }
    ...
    private Event createEvent() throws JSONException {
        if (_event == null)
            _event = JsonFormUtils.createEvent(
                    getFields(),
                    getJSONObject(jsonForm, GizConstants.METADATA),
                    JsonClientProcessingUtils.formTag(allSharedPreferences),
                    entityId,
                    encounterType,
                    bindType
    ...
    public NativeFormProcessor saveEvent() throws JSONException {
        ...
        // inject the details
        if (jsonForm.has(GizConstants.DETAILS))
            eventJson.put(GizConstants.DETAILS, jsonForm.getJSONObject(GizConstants.DETAILS));
    ...
    public NativeFormProcessor mergeAndSaveClient() throws JSONException {
        ...
        JSONObject relationships = mergedJson.optJSONObject(GizConstants.RELATIONSHIPS);
        if ((relationships == null || relationships.length() == 0) && originalClientJsonObject != null) {
            mergedJson.put(GizConstants.RELATIONSHIPS, originalClientJsonObject.optJSONObject(GizConstants.RELATIONSHIPS));
        }
    ...
    public NativeFormProcessor clientProcessForm() throws Exception {
        ...
        GizMalawiProcessorForJava.getInstance(GizMalawiApplication.getInstance().getApplicationContext()).processClient(Collections.singletonList(eventClient));
    ...
    public ECSyncHelper getSyncHelper() {
        return ECSyncHelper.getInstance(GizMalawiApplication.getInstance().getContext().applicationContext());
    }
